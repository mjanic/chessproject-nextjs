...
export default function Board() {
  ...
  const fetchLichessMoves = async (latestFen: string, playerElo: number) => {
    ...
    try {
      ...
      if (data.moves && data.moves.length > 0) {
        ...
        console.log(
          "move1",
          data.moves[1].white + data.moves[1].draws + data.moves[1].black
        );
        if (
          data.moves[0].white + data.moves[0].draws + data.moves[0].black >
            4 *
  ...
  function finishedPractice() {
    ...
    if (engineData.evaluation) {
      if (engineData.evaluation < -1) {
        setCoachDialog(
          `Bravo! You managed to get better position than your opponent in the opening with eval: ${engineData.evaluation}`
        );
      } else if (engineData.evaluation > 1) {
        setCoachDialog(
          `You didnt play well in the opening and finish in the worse position with eval: ${engineData.evaluation}`
        );
      } else {
        setCoachDialog(
          `You fiished in equal position, eval: ${engineData.evaluation}`
        );
      }
    }
  }
  ...
  useEffect(() => {
    ...
    getDataFromStockfish(game.fen()).then(() => {
      console.log(
        "data from stockfish in useeffect after promise",
        useGameStore.getState().engineData
      );
      if (game.turn() !== playerColor[0]) {
        console.log("move num: ", game.moveNumber());
        fetchLichessMoves(game.fen(), playerElo).then(() => {
          ...
          if (updatedLiMoves.length > 0) {
            ...
            const randomIndex = Math.floor(
              Math.random() * updatedLiMoves.length
            );
            ...
            setCoachDialog(
              `Your opponent played ${updatedLiMoves[randomIndex]} which is the ${indexToOrder} played move in this position.`
            );
          } else {
            const updatedEngineData = useGameStore.getState().engineData;
            console.log(
              "engine data in use effect if not limoves: ",
              updatedEngineData
            );
            if (updatedEngineData.bestmove === "") {
              ...
              setCoachDialog(
                "You are continuing against the adapted stockfish level engine because this position is not in the database for your ELO"
              );
            }
          }
        });
  ...
  return (
    <div className="max-w-[400px] flex justify-center flex-col space-y-1 p-2">
      <Coach />
